...
class ApiService
{
    ...
    public function popularEventBatchSecond()
    {
        try {
            ...
            $this->apiRepository->apiConnpass($url);
            $url = 'https://connpass.com/api/v1/event/?count=100&ym=' . $month_after_next . '&order=1&start=1';
            $this->apiRepository->apiAlert($url);
            $url = 'https://connpass.com/api/v1/event/?count=100&ym=' . $month_after_next . '&order=3&start=1';
            $this->apiRepository->apiAlert($url);
            DB::commit();
        } catch (\Throwable $e) {
            DB::rollback();
